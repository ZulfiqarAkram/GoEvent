...
$.extend($.fn, {
	...
	validate: function( options ) {
		/// <summary>
		/// Validates the selected form. This method sets up evento handlers for submit, focus,
...
$.extend($.validator, {
	...
	prototype: {
		...
		init: function() {
			...
			function delegate(evento) {
				var validator = $.data(this[0].form, "validator"),
					eventType = "on" + evento.type.replace(/^validate/, "");
		...
		form: function() {
			...
			/// This behaves as a normal submit evento, but returns the result.
			/// </summary>
			/// <returns type="Boolean" />
		...
		focusInvalid: function() {
			if( this.settings.focusInvalid ) {
				try {
					...
					.filter(":visible")
					.focus()
					// manually trigger focusin evento; without it, focusin handler isn't called, findLastActive won't have anything to find
					.trigger("focusin");
				} catch(e) {
	...
	methods: {
		...
		equalTo: function(value, element, param) {
			// bind to the blur evento of the target in order to revalidate whenever the target field is updated
			// TODO find a way to bind the evento just once, avoiding the unbind-rebind overhead
...
// IE has native support, in other browsers, use evento caputuring (neither bubbles)

// provides delegate(type: String, delegate: Selector, handler: Callback) plugin for easier evento delegation
// handler is only called when $(evento.target).is(delegate), in the scope of the jquery-object for evento.target 
;(function($) {
	// only implement if not provided by jQuery core (since 1.4)
	// TODO verify if jQuery 1.4's implementation is compatible with older jQuery special-evento APIs
	if (!jQuery.evento.special.focusin && !jQuery.evento.special.focusout && document.addEventListener) {
		$.each({
			focus: 'focusin',
			blur: 'focusout'	
			...
			$.evento.special[fix] = {
				...
				handler: function(e) {
					arguments[0] = $.evento.fix(e);
					arguments[0].type = fix;
					return $.evento.handle.apply(this, arguments);
				}
			};
			function handler(e) {
				e = $.evento.fix(e);
				e.type = fix;
				return $.evento.handle.call(this, e);
			}
		});
	};
	$.extend($.fn, {
		validateDelegate: function(delegate, type, handler) {
			return this.bind(type, function(evento) {
